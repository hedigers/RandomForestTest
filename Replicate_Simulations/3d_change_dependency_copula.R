...
Results <- parSim(
  ...
  export = c("S_power", "ntrees","alpha", "K", "rtcopula", "R", "scf_test",
             "nodesize", "me_test","scf_test", "mmd_test", "mmdoptimized_test", "hypoRF"),
  ...
  expression = {
    ...
    Y <- replicate(S_power, cbind(rtcopula(n2,R,df),
                                  replicate(p-d,rnorm(n2))), simplify = F)
    ###################################################################################
...
# write.table(Results, "3d_copula_S200_p200_d20_trees600_node4_K100.txt", sep=";")
# 
# Results <- read.table("3d_copula_S200_p200_d20_trees600_node4_K100.txt", sep=";")
# 
# library(tikzDevice)
# options(tikzDefaultEngine = 'xetex')
# tikz(file = "3d_plot_copula_K100.tex", width=6, height=4)
# 
# par(oma = c(1, 1, 2, 1))
