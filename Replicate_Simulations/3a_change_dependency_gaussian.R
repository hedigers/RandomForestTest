...
n <- 600
p <- 60
cov_value <- seq(0,0.15,by=0.16/16)
ntrees <- 600
alpha <- 0.05
nodesize <- 4 
K <- 100
...
Results <- parSim(
  ...
  export = c("S_power", "ntrees","alpha", "K", "mu_1", "mu_2", "cov_1",
  ...
  expression = {
    ...
    ################################# DATA GENERATION #################################
    cov_2 <- matrix(cov_value, nrow=p, ncol=p)
    diag(cov_2) <- 1
...
# write.table(Results, "3a_gaussian_dep_S200_p60_trees600_node4_K100.txt", sep=";")
# 
# Results <- read.table("3a_gaussian_dep_S200_p60_trees600_node4_K100.txt", sep=";")
# 
# library(tikzDevice)
# options(tikzDefaultEngine = 'xetex')
# tikz(file = "3a_plot_gaussian_dep_K100.tex", width=6, height=4)
# 
# par(oma = c(1, 1, 2, 1))
...
#        pch=c(19,17,15,3,4),
#        cex=1, bty="n", bg="transparent", text.width=c(0.02,0.02,0.02,0.02,0.025))
# 
# dev.off()
