...
#' hypoRF(x1, x2, K=1)
#' @export
hypoRF <- function (data1, data2, K = 100, statistic = "PerClassOOB", normalapprox = F,
          seed = NULL, alpha = 0.05, ...)
{
  if (!is.null(seed) && (!is.numeric(seed) || !is.finite(seed) ||
                         seed < 0)) {
  ...
  if (is.na(statistic) || !(statistic %in% c("PerClassOOB",
                                             "OverallOOB"))) {
  ...
  if (K > 1) {
    ...
    if (statistic == "OverallOOB") {
      ...
      pred_oob <- as.numeric(rf$predictions[, 2] > (n2/(n1 +
                                                          n2)))
      tmp <- cbind(d, pred_oob)[, c("y", "pred_oob")]
      L0 <- sum(tmp[y == 0, "pred_oob"])/length(tmp[y ==
                                                      0, "pred_oob"])
      L1 <- (length(tmp[y == 1, "pred_oob"]) - sum(tmp[y ==
                                                         1, "pred_oob"]))/length(tmp[y == 1, "pred_oob"])
      obs <- L0 + L1
    }
    ...
    for (i in 1:K) {
      ...
      if (statistic == "OverallOOB") {
        ...
        pred_oob <- as.numeric(rf_i$predictions[, 2] >
                                 (n2/(n1 + n2)))
        tmpi <- cbind(d, pred_oob)[, c("y", "pred_oob")]
        L0i <- sum(tmpi[y == 0, "pred_oob"])/length(tmpi[y ==
                                                           0, "pred_oob"])
        L1i <- (length(tmpi[y == 1, "pred_oob"]) - sum(tmpi[y ==
                                                              1, "pred_oob"]))/length(tmpi[y == 1, "pred_oob"])
        val[i] <- L0i + L1i
      }
    ...
    if (c("importance") %in% names(add.param)) {
      if (add.param$importance == "impurity") {
        ...
        cutoff <- stats::quantile(importancedistribution,
                                  1 - alpha)
      }
    }
    else {
